...
def getTitle(ref: bs4.PageElement)->str:
    # get text from inside reference, then replace all whitespace with single spaces, then remove any 'no xyz' suffixes.
    return re.sub(r"no \d+$", r"",re.sub(r"\s+", r" ", "".join([x for x in ref.stripped_strings])), flags=re.MULTILINE|re.IGNORECASE)
